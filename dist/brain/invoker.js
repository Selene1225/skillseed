...
function buildPrompt(content, currentMeta) {
...
export function refineAsync(filePath, content, currentMeta) {
    ...
    try {
        const child = execFile(cmd, args, { timeout: 30000 }, (err, stdout) => {
            if (err)
                return; // Silently fail — rule engine already handled the sync path
            try {
                // Parse LLM response
