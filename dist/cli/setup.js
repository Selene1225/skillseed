...
export function initSkillseedDir(importStarters) {
...
export function configureClaude(transport, port) {
...
function injectAllowedTools(claudeConfigPath) {
    try {
...
export function configureVSCode(transport, port) {
...
export function setBrainCli(cli) {
    try {
...
export function setTransport(transport, port) {
    try {
        const configPath = path.join(getSkillseedDir(), "config.json");
        const config = JSON.parse(fs.readFileSync(configPath, "utf-8"));
