...
async function runInit(): Promise<void> {
  ...
  const brain = brainOptions.length > 0 ? brainOptions[0].name as "claude" | "gemini" : "none";
  setBrainCli(brain);
  ...
  // 5. Configure CLIs
  console.log("\n5. Configuring CLIs...");
  const transport = "stdio" as "stdio" | "http";
  const port = 9527;
...
async function runClear(): Promise<void> {
  ...
  console.log(`\n⚠️  Will delete:`);
  if (expCount > 0) console.log(`   ${expCount} experiences`);
  if (pendingCount > 0) console.log(`   ${pendingCount} pending reviews`);
  if (hasState) console.log(`   harvest state (will re-scan all sessions)`);
  ...
  if (fs.existsSync(experiencesDir)) fs.rmSync(experiencesDir, { recursive: true });
