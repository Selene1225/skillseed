...
import { getSkillseedDir, writeExperience, listAllExperiences, moveExperienceScope, type ExperienceFrontmatter } from "../store/file-store.js";
...
export function parseJsonlFile(filePath: string): ConversationTurn[] {
...
export function scoreChunk(chunk: ConversationChunk): number {
  let score = 0;
...
export function chunkConversation(turns: ConversationTurn[], file: string, project: string): ConversationChunk[] {
  const chunks: ConversationChunk[] = [];
  const CHUNK_SIZE = 20;

  const substantiveTurns = turns.filter(t => !isSkippable(t.text));
  if (substantiveTurns.length < 4) return chunks;
...
export function writePending(exp: PendingExperience, brainCli: string = "claude"): string | null {
...
export function listPending(): Array<{ file: string; content: string; meta: Record<string, string> }> {
  const dir = getPendingDir();
...
export function harvest(opts: HarvestOptions = {}): { scanned: number; extracted: number; pending: number } {
  ...
  for (const f of files) {
    ...
    state[f.file] = f.mtime;
    if (!opts.dryRun) saveHarvestState(state);
  }
